...
const generateRow = (i, id, type, myChannel) => {
  ...
  r[0].setCustomId(`commandsSlash/settings/channel.js ${i.member.id} ${id} ${type} noXp`);
  r[0].setStyle(myChannel.noXp ? 'SUCCESS' : 'DANGER');

  r[1].setCustomId(`commandsSlash/settings/channel.js ${i.member.id} ${id} ${type} noCommand`);
  ...
  r[2].setCustomId(`commandsSlash/settings/channel.js ${i.member.id} ${id} ${type} commandOnly`);
...
const _close = (i) => new MessageActionRow()
  .addComponents(new MessageButton()
    .setLabel('Close')
    .setStyle('DANGER')
    .setCustomId(`commandsSlash/settings/channel.js ${i.member.id} - - closeMenu`));
