...
module.exports.execute = async function(i) {
  ...
  const roleDeassignmentsByLevel =
    await guildRoleModel.storage.getRoleDeassignmentsByLevel(i.guild, 'deassignLevel', lvl);

  if (lvl != 0 && roleDeassignmentsByLevel.length >= 3) {
    return await i.reply({
      content: 'There is a maxiumum of 3 role deassignments per level. Please remove some first.',
      ephemeral: true,
    });
  }

  await guildRoleModel.storage.set(i.guild, i.options.getRole('role'), 'deassignLevel', lvl);
