...
const generateRow = (i, id, myRole) => {
  ...
  r[0].setCustomId(`commandsSlash/settings/role-options.js ${i.member.id} ${id} noXp`);
  r[0].setStyle(myRole.noXp ? 'SUCCESS' : 'DANGER');

  r[1].setCustomId(`commandsSlash/settings/role-options.js ${i.member.id} ${id} assignMessage`);
  r[1].setStyle('SECONDARY');

  r[2].setCustomId(`commandsSlash/settings/role-options.js ${i.member.id} ${id} deassignMessage`);
...
const _close = (i) => new MessageActionRow()
  .addComponents(new MessageButton()
    .setLabel('Close')
    .setStyle('DANGER')
    .setCustomId(`commandsSlash/settings/role-options.js ${i.member.id} - closeMenu`));

const _modal = (roleId, assignState) => new Modal()
  .setCustomId(`commandsSlash/settings/role-options.js ${roleId} ${assignState ? 'assignMessage' : 'deassignMessage'}`)
