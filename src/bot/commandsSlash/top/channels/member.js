...
module.exports.execute = async (i) => {
  ...
  const header = `${_prettifyType[type]} channel toplist for ${guildMemberInfo.name} from ${page.from} to ${page.to} | ${_prettifyTime[time]}`;
  ...
  const embed = new EmbedBuilder().setTitle(header).setColor("#4fd6c8");
  ...
  for (let iter = 0; iter < guildMemberTopChannels.length; iter++) {
    if (type == "voiceMinute")
      str =
        ":microphone2: " +
        Math.round((guildMemberTopChannels[iter][time] / 60) * 10) / 10;
    ...
    embed.addFields({
      name: `#${page.from + iter} ${nameUtil.getChannelName(
        ...
        guildMemberTopChannels[iter].channelId
      )}`,
      value: str,
    });
  }
