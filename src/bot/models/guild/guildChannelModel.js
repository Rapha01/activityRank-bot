...
exports.cache.load = (channel) => {
  if(!channel.appData) {
    if(promises[channel.id]) { return promises[channel.id]; }
    ...
    promises[channel.id] = new Promise(async (resolve,reject) => {
      try {
        ...
        resolve();
      } catch (e) { delete promises[channel.id]; reject(e); }
    });
...
exports.storage.get = (guild,channelId) => {
  return new Promise(async function (resolve, reject) {
    try {
      const res = await shardDb.query(guild.appData.dbHost,`SELECT * FROM guildChannel WHERE guildId = ${guild.id} && channelId = ${mysql.escape(channelId)}`);

      if (res.length == 0) {
        if (!defaultAll) defaultAll = (await shardDb.query(guild.appData.dbHost,`SELECT * FROM guildChannel WHERE guildId = 0 AND channelId = 0`))[0];
...
exports.storage.set = (guild,channelId,field,value) => {
  return new Promise(async function (resolve, reject) {
    try {
      await shardDb.query(guild.appData.dbHost,`INSERT INTO guildChannel (guildId,channelId,${field}) VALUES (${guild.id},${mysql.escape(channelId)},${mysql.escape(value)}) ON DUPLICATE KEY UPDATE ${field} = ${mysql.escape(value)}`);
...
exports.getNoXpChannelIds = (guild) => {
  return new Promise(async function (resolve, reject) {
    try {
      const res = await shardDb.query(guild.appData.dbHost,`SELECT channelId FROM guildChannel WHERE guildId = ${guild.id} AND noXp = 1`);
...
exports.getNoCommandChannelIds = (guild) => {
  return new Promise(async function (resolve, reject) {
    try {
      const res = await shardDb.query(guild.appData.dbHost,`SELECT channelId FROM guildChannel WHERE guildId = ${guild.id} AND noCommand = 1`);
...
const buildCache = (channel) => {
  return new Promise(async function (resolve, reject) {
    try {
      let cache = await shardDb.query(channel.guild.appData.dbHost,`SELECT ${cachedFields.join(',')} FROM guildChannel WHERE guildId = ${channel.guild.id} AND channelId = ${channel.id}`);
...
const loadDefaultCache = (dbHost) => {
  return new Promise(async function (resolve, reject) {
    try {
      let res = await shardDb.query(dbHost,`SELECT ${cachedFields.join(',')} FROM guildChannel WHERE guildId = 0 AND channelId = 0`);
