...
const buildCache = (guild) => {
  return new Promise(async function(resolve, reject) {
    try {
      ...
      if (cache.length == 0) {
        console.log;
        await shardDb.query(dbHost, `INSERT INTO guild (guildId,joinedAtDate,addDate) VALUES (${guild.id},${Math.floor(guild.members.me.joinedAt / 1000)},${Math.floor(Date.now() / 1000)})`);
