  ...
  return new Promise(async function (resolve, reject) {
    try {
      ...
      if (affectedRows < batchsize)
        affectedRows += await exports.storage.resetGuildSettings(
          batchsize,
          guild
        );
    ...
    try {
      ...
      affectedRows += (
        await shardDb.query(
          ...
          `DELETE FROM guildRole WHERE guildId = ${guild.id} LIMIT ${batchsize}`
        )
      ).affectedRows;
      if (affectedRows < batchsize)
        affectedRows += (
          await shardDb.query(
            guild.appData.dbHost,
            `DELETE FROM guildMember WHERE guildId = ${guild.id} LIMIT ${
              batchsize - affectedRows
            }`
          )
        ).affectedRows;
      if (affectedRows < batchsize)
        affectedRows += (
          await shardDb.query(
            guild.appData.dbHost,
            `DELETE FROM guildChannel WHERE guildId = ${guild.id} LIMIT ${
              batchsize - affectedRows
            }`
          )
        ).affectedRows;
  ...
  return new Promise(async function (resolve, reject) {
    try {
      ...
      affectedRows += (
        await shardDb.query(
          guild.appData.dbHost,
          `DELETE FROM textMessage WHERE guildId = ${guild.id} LIMIT ${batchsize}`
        )
      ).affectedRows;
      if (affectedRows < batchsize)
        affectedRows += (
          await shardDb.query(
            guild.appData.dbHost,
            `DELETE FROM voiceMinute WHERE guildId = ${guild.id} LIMIT ${
              batchsize - affectedRows
            }`
          )
        ).affectedRows;
      if (affectedRows < batchsize)
        affectedRows += (
          await shardDb.query(
            guild.appData.dbHost,
            `DELETE FROM vote WHERE guildId = ${guild.id} LIMIT ${
              batchsize - affectedRows
            }`
          )
        ).affectedRows;
      if (affectedRows < batchsize)
        affectedRows += (
          await shardDb.query(
            guild.appData.dbHost,
            `DELETE FROM invite WHERE guildId = ${guild.id} LIMIT ${
              batchsize - affectedRows
            }`
          )
        ).affectedRows;
      if (affectedRows < batchsize)
        affectedRows += (
          await shardDb.query(
            guild.appData.dbHost,
            `DELETE FROM bonus WHERE guildId = ${guild.id} LIMIT ${
              batchsize - affectedRows
            }`
          )
        ).affectedRows;
  ...
  return new Promise(async function (resolve, reject) {
    try {
      ...
      affectedRows += (
        await shardDb.query(
          guild.appData.dbHost,
          `DELETE FROM textMessage WHERE guildId = ${
            guild.id
          } AND userId IN (${userIds.join(",")}) LIMIT ${batchsize}`
        )
      ).affectedRows;
      if (affectedRows < batchsize)
        affectedRows += (
          await shardDb.query(
            guild.appData.dbHost,
            `DELETE FROM voiceMinute WHERE guildId = ${
              guild.id
      ...
      if (affectedRows < batchsize)
        affectedRows += (
          await shardDb.query(
            guild.appData.dbHost,
            `DELETE FROM vote WHERE guildId = ${
              guild.id
            } AND userId IN (${userIds.join(",")}) LIMIT ${
              batchsize - affectedRows
            }`
      ...
      if (affectedRows < batchsize)
        affectedRows += (
          await shardDb.query(
            guild.appData.dbHost,
            `DELETE FROM invite WHERE guildId = ${
              guild.id
            } AND userId IN (${userIds.join(",")}) LIMIT ${
              batchsize - affectedRows
            }`
      ...
      if (affectedRows < batchsize)
        affectedRows += (
          await shardDb.query(
            guild.appData.dbHost,
            `DELETE FROM bonus WHERE guildId = ${
              guild.id
            } AND userId IN (${userIds.join(",")}) LIMIT ${
              batchsize - affectedRows
            }`
    ...
    try {
      ...
      affectedRows += (
        await shardDb.query(
          ...
          `DELETE FROM textMessage WHERE guildId = ${
            guild.id
          } AND channelId IN (${channelIds.join(",")}) LIMIT ${batchsize}`
        )
      ).affectedRows;
      if (affectedRows < batchsize)
        affectedRows += (
