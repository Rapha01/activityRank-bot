...
module.exports.data = new SlashCommandBuilder()
  ...
  .addSubcommand(sc => sc
    ...
    .addBooleanOption(o => o
      .setName('independent')
      .setDescription('If selected, will not count tokens as purchased. Also applies for negative changes.')));

module.exports.execute = async function(i) {
  if (!authorizedUsers.includes(i.user.id)) {
    console.log(`Unauthorized command attempt in editTokens command by user ${i.user.tag} [${i.user.id}]`);
    return;
  }
  ...
  if (!value) return await i.reply({ content: 'Invalid `amount` param', ephemeral: true });
  ...
  if (!indep)
    await userModel.storage.increment(target, 'tokensBought', value);

  await i.reply({
    content: `<@${target.id}>'s tokens were changed by \`${value.toLocaleString()}\`. \n*This process __${
      indep ? 'did not' : 'did'
