const { SlashCommandBuilder, PermissionFlagsBits, AttachmentBuilder, ButtonStyle, ActionRowBuilder, ButtonBuilder, EmbedBuilder, Status } = require('discord.js');
...
module.exports.data = new SlashCommandBuilder()
  ...
  .addIntegerOption(o => o
    .setName('page')
    .setDescription('Find a page').setMaxValue(200))
  .addIntegerOption(o => o
    .setName('search')
...
module.exports.execute = async function(i) {
  ...
  let data = await i.client.shard.broadcastEval(client => [client.shard.ids, client.ws.status, client.ws.ping, client.guilds.cache.size]);
  data = data.map(shard => ({ ids: shard[0], status: shard[1] === 0 ? '🟢 Online' : `🔴 ${Status[shard[1]]}`, ping: shard[2], guilds: shard[3] }));

  if (filtered) 
    data = data.filter(({ status }) => status !== '🟢 Online')
  ...
  if (Number.isInteger(search)) {
    ...
    return await i.editReply({
      content: found ? `Shard ${found.ids}:\n  **Status:** ${found.status}\n  **Ping:** ${found.ping}ms\n  **Guilds:** ${found.guilds}` : 'Could not find shard',
      ephemeral,
      files,
    })
  }
  ...
  const e = new EmbedBuilder().setTitle(`Shards ${page * 15} - ${(page + 1) * 15} (total ${data.length})`)
  .setFields(data.slice(page * 15, (page + 1) * 15).map(
    s => ({ name: s.ids.join(', '), value: `**Status:** ${s.status}\n**Ping:** ${s.ping}ms\n**Guilds:** ${s.guilds}`, inline: true })
  ))
