...
exports.addTextMessage = (member,channel,count) => {
  return new Promise(async function (resolve, reject) {
    try {
      ...
      if (!entry)
        entry = textMessageCache[member.id + channel.id] = {guildId:member.guild.id,userId:member.id,channelId:channel.id,count:count};
      else
        entry.count += count;
...
exports.addVoiceMinute = (member,channel,count) => {
  return new Promise(async function (resolve, reject) {
    try {
      ...
      if (!entry)
        entry = voiceMinuteCache[member.id + channel.id] = {guildId:member.guild.id,userId:member.id,channelId:channel.id,count:count};
      else
        entry.count += count;
      ...
      if (member.guild.appData.bonusUntilDate > Date.now() / 1000)
        await exports.addBonus(member,count * member.guild.appData.bonusPerVoiceMinute);
...
exports.addVote = (member,count) => {
  return new Promise(async function (resolve, reject) {
    try {
      ...
      if (!entry)
        entry = voteCache[member.id] = {guildId:member.guild.id,userId:member.id,count:count};
      else
        entry.count += count;
...
exports.addBonus = (member,count) => {
  return new Promise(async function (resolve, reject) {
    try {
      ...
      if (!entry)
        entry = bonusCache[member.id] = {guildId:member.guild.id,userId:member.id,count:count};
      else
        entry.count += count;
...
const buildStatFlushCache = (member,type) => {
  ...
  if (!statFlushCache[dbHost])
    statFlushCache[dbHost] = {};

  if (!statFlushCache[dbHost][type])
