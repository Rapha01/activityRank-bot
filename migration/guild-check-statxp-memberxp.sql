...
-- This script will check that a all members in a guild's XP values stored in the `guildMember` table
...
-- It returns a list of userIds and their corresponding booleans: `checkAlltime`, `checkYear`, `checkMonth`, `checkWeek`, `checkDay`, and `checkAll`.
...
FROM (SELECT
  ...
  LEFT JOIN (SELECT userId, 
        ...
        week as memberWeek,
        day as memberDay
        FROM guildMember WHERE guildId = {{guildId}}) AS memberrank ON userIds.userId = memberrank.userId)
