<script lang="ts">
  ...
  const dispPermission = $derived.by(() => nameApiPermission(permission));
</script>
...
<Popover.Root>
  ...
  <Popover.Portal>
    <Popover.Content side="top" sideOffset={4}>
      ...
      <div class="rounded-md bg-slate-200 dark:bg-slate-800 shadow-md border-2 border-slate-950/50 dark:border-white/50 py-1 px-2 text-sm font-medium text-slate-950 dark:text-slate-100">
        {dispPermission}
      </div>
    </Popover.Content>
  </Popover.Portal>
